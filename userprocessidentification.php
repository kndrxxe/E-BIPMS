...
if (empty($_POST['firstname']) || empty($_POST['lastname'])) {
  ...
  header('Location: useridentification.php');
} else {
  $userID = $_POST['userID'];
  $status = $_POST['status'];
  ...
  $query = "SELECT * FROM identification WHERE status = 0 AND userID = '$uid'";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    $query = "SELECT * FROM identification WHERE userID = '$userID' AND status = 0";
    $result = mysqli_query($conn, $query);
